...
// and try to use it.
public class swbasicenemy : BasicAICreature
{
    ...
    public List<GameObject> ammoDropPrefabs;
    ...
    {
        int ammoToDrop = Random.Range(1, 4);
        for (int i = 0; i < ammoToDrop; i++)
        {
            GameObject selectedAmmoPrefab = ammoDropPrefabs[Random.Range(0, ammoDropPrefabs.Count)];
            Instantiate(selectedAmmoPrefab, transform.position, Quaternion.identity);
        }
    }
